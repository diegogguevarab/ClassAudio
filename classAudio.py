...
# Waveshow
data, sampling_rate = librosa.load(path_audio_files + "blues/blues.00000.wav")
...
data_predict, sampling_rate_predict = librosa.load(
    "./Data/genres_original/metal/rock.00023.wav")
...
spectral_rolloff_predict = librosa.feature.spectral_rolloff(
    data_predict, sr=sampling_rate)[0]
...
X_predict = scaler.fit_transform(np.array(predict_df.iloc[:, :], dtype=float))
...
voting_predict = voting_c.predict(X_predict)
